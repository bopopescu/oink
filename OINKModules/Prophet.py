...
class Prophet(QtCore.QThread):
	...
	def __init__(self, user_id, password, active_date=None, start_date=None, end_date=None, mode=None):
		...
		if start_date is None:
			self.start_date = getFirstDisplayedDateFor(self.active_date)
		else:
			self.start_date = start_date
		
		if end_date is None:
			self.end_date = getLastDisplayedDateFor(self.active_date)
		else:
			self.end_date = end_date
		
		if mode is not in [1,2,3]:
			self.mode = 1
		else:
			self.mode = mode	
	...
	def broadcastCalendarData(self):
        ...
        if self.current_calendar_data != self.current_calendar_data:
	...
	def updateWorkCalendar(self, status, relaxation, comment, start_date, end_date):
		dates = self.getWorkingDatesBetween(start_date, end_date)
		#First, check if there's an entry for that date.
		#If it exists, update.
		#Else, add an entry.
		status = True
		return status
    ...
    def readFromOINK(self, sqlcmdstring, function_name):
	    ...
	    while (trial < 3) and (try_again):
	    	try:
	    		...
	    		print "*********************"
	        	print "Error in Prophet. Function name %s." %
	        	print "Time: %s" datetime.datetime.now()
	...
	def getFirstDisplayedDateFor(self):
