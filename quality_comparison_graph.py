...
if __name__ == "__main__":
 ...
 #   writer_quality = 100*np.array(writer_quality)
    ...
    x = np.array([y+1 for y in np.arange(len(team_quality))])
    #print x
    x1 = np.arange(len(team_quality))
...
#    plt.barh(x, writer_quality, height=0.5, alpha=0.45, color="blue", linestyle='solid', label="%s"%writer_name[:writer_name.find(" ")])
    ...
    plt.title("Team Quality Parameter-Wise Break Up Between %s and %s" %(d1, d2))
