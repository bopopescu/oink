...
def main():
    ...
    try:
        ...
        if login_details is not None:
            ...
            user_list = MOSES.getEmployeesList(userID, password, datetime.date.today())
            access_list = list(user_list[user_list["Employee ID"] == userID]["OINK Access Level"])[0].split(",")
            user_role = list(user_list[user_list["Employee ID"] == userID]["Role"])[0]
            ...
            if len(access_list)>0:
                ...
                if user_role not in userDict.keys():
                    ...
                    if len(access_list)>1:
                        window = OINKChooser(userID, password, access_list)
                    else:
